...
Studio asks for an **IAM role name** (for example `HighflameDiscovery`), not a full Role ARN. It builds `arn:aws:iam::<account_id>:role/<name>` from the account ID on the same form.
...
Verify that the role name matches what you entered in Studio. Highflame sends discovery this ARN (you do not type it):
...
| **IAM role name** | `HighflameDiscovery` (Studio builds `arn:aws:iam::<TARGET-ACCOUNT-ID>:role/<name>`) |
